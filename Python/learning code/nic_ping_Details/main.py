...
class MyForm(wx.Frame):
    ...
    def runDiagnostics(self):
        ...
        "Route Table:"
        print
        "%20s\t %15s\t %15s\t %15s\t %s" % ("Network Destination", "Netmask",
                                            "Gateway", "Interface", "Metric")
        for route in route_table:
            if len(route) == 5:
                dst, mask, gateway, interface, metric = route
                print
                "%20s\t %15s\t %15s\t %15s\t %s" % (dst, mask, gateway, interface, metric)
    ...
    def getNICInfo(self):
        ...
        for objItem in colItems:
            ...
            print
            "***Interface %s of %s***" % (count, numOfNics)
            if objItem.IPEnabled == True:
                ...
                print
                "DHCP Lease Obtained: ", objItem.DHCPLeaseObtained
                print
                ...
                print
                "WINS Enable LMHosts Lookup: ", objItem.WINSEnableLMHostsLookup
                print
